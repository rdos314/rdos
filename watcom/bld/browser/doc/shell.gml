:INCLUDE file='FMTMACRO'.
:INCLUDE file='GMLMACS'.
:GDOC.
.* ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
.* ³ Copyright by WATCOM Systems Inc. 1991.  All rights reserved.  ³
.* ³ No part of this publication may be reproduced in any form or  ³
.* ³ by any means - graphic, electronic or mechanical, including   ³
.* ³ photocopying, recording, taping or information storage and    ³
.* ³ retrieval systems. 					   ³
.* ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
.*
:FRONTM.
:TITLEP.
:TITLE stitle="User's Guide".Browser
:TITLE.User's Guide
:set symbol="isbn" value="1-55094-022-8"
:AUTHOR.P. Harapiak
:ADDRESS.
:ALINE.WATCOM International Corporation
:ALINE.Waterloo, Ontario, Canada
:eADDRESS.
:cmt. :graphic depth='0.7i' xoff='-2.55i' yoff='0.0i' scale=50 file='watcom.eps'.
:eTITLEP.
:TOC.
:FIGLIST.
:BODY.
:cmt.sepsect &cmpname User's Guide
:cmt.:INCLUDE file='WBROWSE'.
:INCLUDE file='WBR'.
:BACKM.
:INDEX.
:eGDOC.
