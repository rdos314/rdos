...
#
##########################################################################*/
void TQuiz6::WriteLongName(TFile &File)
{
...
void TQuiz6::SetupTexts()
{
	...
	Quiz[44].MyGroup = GROUP_ASPIE_OBSESSION;
	...
	Quiz[53].MyGroup = GROUP_ASPIE_OBSESSION;
	Quiz[54].MyGroup = GROUP_ASPIE_OBSESSION;
	Quiz[55].MyGroup = GROUP_ENVIRONMENT;
	Quiz[56].MyGroup = GROUP_ASPIE_OBSESSION;
	Quiz[57].MyGroup = GROUP_ASPIE_OBSESSION;
	Quiz[58].MyGroup = GROUP_ASPIE_OBSESSION;
	...
	Quiz[63].MyGroup = GROUP_ENVIRONMENT;
	...
	Quiz[79].MyGroup = GROUP_NT_HUNTING;
	...
	Quiz[85].MyGroup = GROUP_SEX;
	Quiz[86].MyGroup = GROUP_MIXED;
	Quiz[87].MyGroup = GROUP_SOCIAL;
	Quiz[88].MyGroup = GROUP_NT_OBSESSION;
	Quiz[89].MyGroup = GROUP_NT_OBSESSION;
	Quiz[90].MyGroup = GROUP_NT_OBSESSION;
	Quiz[91].MyGroup = GROUP_NT_OBSESSION;
	...
	Quiz[101].MyGroup = GROUP_DYSC;
	Quiz[102].MyGroup = GROUP_NT_TALENT;
	Quiz[103].MyGroup = GROUP_DYSC;
	Quiz[104].MyGroup = GROUP_NT_TALENT;
	Quiz[105].MyGroup = GROUP_DYSC;
	Quiz[106].MyGroup = GROUP_DYSC;
	Quiz[107].MyGroup = GROUP_NT_TALENT;
	Quiz[108].MyGroup = GROUP_DYSC;
	Quiz[109].MyGroup = GROUP_SOCIAL;
	Quiz[110].MyGroup = GROUP_NT_OBSESSION;
	...
	Quiz[119].MyGroup = GROUP_NT_OBSESSION;
...
void TQuiz6::SetupCross(TQuiz *QuizI, TQuiz *QuizII, TQuiz *QuizIII, TQuiz *QuizNd, TQuiz *Quiz5)
{
	...
	DefineGlobalId(    150, 456);
	DefineCross(QuizIII, 151, 92);
}
...
*   Created....: 96-11-20 le                                                #
*##########################################################################*/
void TQuiz6::ExportExcelAspie(const char *filename)
{
	TQuizRow Row;
	int i;
	int ival;
...
void THair::Add(TQuizRow *Row)
{
    ...
    if (Row->Ancestry >= 2000 && Row->Ancestry < 3000)
    {
	    if (diff > 0)
		    AsCount[1][index]++;
    	else
...
void TRace::Add(TQuizRow *Row)
{
	...
	if (Row->Ancestry == 5)
	    index = 1;      // afroamerican

	if (Row->Ancestry == 6)
		index = 2;      // hispanic

	if (Row->Ancestry >= 1000 && Row->Ancestry < 2000)
		index = 5;      // black african
...
void TQuiz6::WriteHair(const char *filename)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		switch (i)
		{
			...
			case 1:
				file.Write("Hair-color, Caucasians");
				break;
...
void TQuiz6::WriteEye(const char *filename)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		switch (i)
		{
			...
			case 1:
				file.Write("Eye color, Caucasians");
				break;
...
void TQuiz6::WriteRace(const char *filename)
{
    ...
    race.UsCount[1] = race.NonUsCount[1];
    race.NonUsCount[1] = 0;
    
    ival = race.NonUsCount[3];
    race.UsCount[3] = 45 * ival / 100;
    race.NonUsCount[3] = 55 * ival / 100;

    ival = race.NonUsCount[4];
    race.UsCount[4] = 35 * ival / 100;
    race.NonUsCount[4] = 65 * ival / 100;

    ival = race.NonUsCount[0];
    race.UsCount[0] = 70 * ival / 100;
    race.NonUsCount[0] = 30 * ival / 100;

    ival = race.NonUsCount[2];
    race.UsCount[2] = 65 * ival / 100;
    race.NonUsCount[2] = 35 * ival / 100;
    ...
    race.WriteNonUsRow(file, 3, "Caucasian");
    race.WriteNonUsRow(file, 4, "Asian");
    race.WriteNonUsRow(file, 5, "Black African");
    race.WriteNonUsRow(file, 6, "Arab");
