...
#
##########################################################################*/
void TQuizI::GetDxData(int PopType, int GroupArr[MAX_GROUP_COUNT], int Arr[MAX_SCORE][2], int OnlyNtControl)
{
	TQuizRow Row;
	int res;
	int g;
    	...
    	if (res >= 0 && res < MAX_SCORE)
	    {
		    ...
		    switch (PopType)
		    {
				case POP_TYPE_AS:
					 ...
					 if (NoDx && (Row.Diagnos == DX_UNKNOWN))
						  Arr[res][0]++;
					 break;
...
void TQuizI::SetupTexts()
{
	...
	Quiz[37].MyGroup = GROUP_ASPIE_OBSESSION;
	...
	Quiz[61].MyGroup = GROUP_ASPIE_SENSORY;
