...
void TQuizNd::SetupTexts()
{
	...
	Quiz[101].MyGroup = GROUP_ASPIE_COMM;
	...
	Quiz[161].MyGroup = GROUP_NT_TALENT;
	Quiz[162].MyGroup = GROUP_NT_TALENT;
	Quiz[163].MyGroup = GROUP_NT_TALENT;
	Quiz[164].MyGroup = GROUP_NT_TALENT;
	...
	Quiz[175].MyGroup = GROUP_ASPIE_COMM;
	...
	Quiz[182].MyGroup = GROUP_ASPIE_COMM;
	Quiz[183].MyGroup = GROUP_ASPIE_BIOLOGY;
	Quiz[184].MyGroup = GROUP_SENSORY;
	Quiz[185].MyGroup = GROUP_SENSORY;
	Quiz[186].MyGroup = GROUP_SENSORY;
	...
	Quiz[200].MyGroup = GROUP_SENSORY;
	Quiz[201].MyGroup = GROUP_NT_BIOLOGY;
	Quiz[202].MyGroup = GROUP_SENSORY;
	Quiz[203].MyGroup = GROUP_NT_TALENT;
	Quiz[204].MyGroup = GROUP_SENSORY;
...
void TQuizNd::LoadPopulations()
{
	...
	while (FDataFile.Read(&Row, sizeof(Row)))
	{
		...
		if (Row.Hyperlexia)
			Hyperlexia.Add(Row.AsResult, Row.NtResult, aspie, Row.Quiz, Row.GroupResult);

		if (Row.Dyspraxia)
			Dyspraxia.Add(Row.AsResult, Row.NtResult, aspie, Row.Quiz, Row.GroupResult);
		...
		if (Row.Synaesthesia)
			Synaesthesia.Add(Row.AsResult, Row.NtResult, aspie, Row.Quiz, Row.GroupResult);

		if (Row.PA)
			PA.Add(Row.AsResult, Row.NtResult, aspie, Row.Quiz, Row.GroupResult);

		if (Row.Dysgraphia)
			Dysgraphia.Add(Row.AsResult, Row.NtResult, aspie, Row.Quiz, Row.GroupResult);
