...
#
# convl9.cpp
# Convert exported quiz-l9 to binary file
#
########################################################################*/
#include <stdio.h>
#include <string.h>
...
static void HandleRow(TQuizRow *Row)
{
    ...
    printf("L9: %d AS: %d, NT: %d", Row->ID, Row->AsResult, Row->NtResult);
    printf("\r\n");
}
...
static void ProcessRow(char *str)
{
    ...
    for (fieldno = 0; ptr; fieldno++)
    {
        ...
        switch (fieldno)
        {
            ...
            case 15:
                 Row.LshScenario = atoi(valstr);
                 break;

            case 16:
                 Row.Quiz[121] = atoi(valstr);
                 break;
...
void ConvL9()
{
    ...
    int size;
    long pos = 0;
    TFile infile("raw\\aspie-quiz-l9.csv");
    TFile outfile("bin\\quizl9.bin", 0);
    char *ptr;
