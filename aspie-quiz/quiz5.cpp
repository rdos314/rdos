...
#
##########################################################################*/
void TQuiz5::WriteLongName(TFile &File)
{
...
#
##########################################################################*/
void TQuiz5::GetDxData(int PopType, int GroupArr[MAX_GROUP_COUNT], int Arr[MAX_SCORE][2], int OnlyNtControl)
{
	TQuizRow Row;
	int res;
	int g;
...
void TQuiz5::SetupTexts()
{
	...
	Quiz[43].MyGroup = GROUP_ASPIE_OBSESSION;
	...
	Quiz[78].MyGroup = GROUP_NT_OBSESSION;
	...
	Quiz[83].MyGroup = GROUP_NT_SENSORY;
	Quiz[84].MyGroup = GROUP_NT_TALENT;
	Quiz[85].MyGroup = GROUP_NT_HUNTING;
	...
	Quiz[94].MyGroup = GROUP_NT_HUNTING;
...
void TQuiz5::LoadPopulations()
{
	...
	while (FDataFile.Read(&Row, sizeof(Row)))
	{
		...
		if (Row.IqResult <= 10)
			LowIQ.Add(Row.AsResult, Row.NtResult, aspie, Row.Gender, Row.Quiz, Row.GroupResult);

		if (Row.IQ >= 120)
			HighIQ.Add(Row.AsResult, Row.NtResult, aspie, Row.Gender, Row.Quiz, Row.GroupResult);
