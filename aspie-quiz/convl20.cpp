...
#
# convl20.cpp
# Convert exported quiz-l20 to binary file
#
########################################################################*/
#include <stdio.h>
#include <string.h>
...
static void HandleRow(TQuizRow *Row)
{
    ...
    printf("L20: %d AS: %d, NT: %d", Row->ID, Row->AsResult, Row->NtResult);
    printf("\r\n");
}
    ...
    for (fieldno = 0; ptr; fieldno++)
    {
        ...
        switch (fieldno)
        {
            ...
            default:
                 ...
                 if (i < 30)
                     Row.Quiz[122 + i] = atoi(valstr);
                 else
                     Row.Quiz[i - 30] = atoi(valstr);
                 break;
        }
    }
...
void ConvL20()
{
    ...
    int size;
    long pos = 0;
    TFile infile("raw\\aspie-quiz-l20.csv");
    TFile outfile("bin\\quizl20.bin", 0);
    char *ptr;
