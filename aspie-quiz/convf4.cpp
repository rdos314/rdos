...
#
# convf4.cpp
# Convert exported quiz-f4 to binary file
#
########################################################################*/
#include <stdio.h>
#include <string.h>
...
const char InsertString[] = "INSERT INTO aspie-quiz-f4 VALUES(";

TFile quizfile("quizf4.bin", 0);
TFile ancfile("ancf4.bin", 0);
...
char *ProcessRow(char *str)
{
	...
	for (fieldno = 0; fieldno < 173; fieldno++)
	{
		valstr = str;
		...
		if (*ptr == ',' || *ptr == ')')
		{
			*ptr = 0;
			...
			switch (fieldno)
			{
				...
				case 12:
					Row.PartnerID = atol(valstr);
					break;

				case 13:
					Row.PartnerAs = atoi(valstr);
					break;

				case 14:
					Row.PartnerNt = atoi(valstr);
					break;
