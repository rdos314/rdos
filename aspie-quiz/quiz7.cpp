...
void TQuiz7::SetupTexts()
{
	...
	Quiz[101].MyGroup = GROUP_SEX;
	Quiz[102].MyGroup = GROUP_SEX;
	Quiz[103].MyGroup = GROUP_SEX;
	Quiz[104].MyGroup = GROUP_INTROVERT;
	Quiz[105].MyGroup = GROUP_INTROVERT;
	Quiz[106].MyGroup = GROUP_INTROVERT;
	Quiz[107].MyGroup = GROUP_INTROVERT;
	Quiz[108].MyGroup = GROUP_INTROVERT;
	...
	Quiz[77].Text = "Do you tend to interpret things literally and/or reply to rhetorical questions?";
	...
	Quiz[89].Text = "Do you know when you are expected to offer an apology?";
	Quiz[90].Text = "Do you feel stress, panic or have a brain malfunction in unfamiliar or demanding situations?";
	Quiz[91].Text = "Do you love to collect and organize things, make lists & diagrams etc?";
	Quiz[92].Text = "Do you prefer to wear the same clothes and/or eat the same food every day?";
	Quiz[93].Text = "Do you need to sit on your favourite seat, go the same route or shop in the same shop every time?";
	Quiz[94].Text = "Do you become frustrated if an activity that is important to you gets interrupted?";
	Quiz[95].Text = "Does it cause chaos in your body or mind if your plans, environment or daily routines suddenly get changed?";
	Quiz[96].Text = "Do you have certain simple & logical routines which you need to follow?";
	Quiz[97].Text = "Do you have a need for comfort items like blankets, stuffed animals etc?";
	Quiz[98].Text = "Before doing something or going somewhere, do you need to have a picture in your mind of what's going to happen so as to be able to prepare yourself mentally first?";
	Quiz[99].Text = "Do you have very strong attachments to certain objects?";
	Quiz[100].Text= "Do you have obsessions or compulsions (repeated irresistible impulses to do certain things)?";
	Quiz[101].Text= "Do you feel like you were born with the wrong gender?";
	Quiz[102].Text= "Do you have unusual sexual preferences?";
	...
	Quiz[73].Text = "Pratar du med dig själv?";
	Quiz[74].Text = "Använder du små ljud som andra inte verkar använda i samtal?";
	Quiz[75].Text = "Har du blivit anklagad för att glo?";
	Quiz[76].Text = "I samtal, brukar du ha problem med saker som timing, turtagning och ömsesidighet?";
	Quiz[77].Text = "Har du en tendens att tolka saker bokstavligt och/eller svara på retoriska frågor?";
	...
	Quiz[85].Text = "Glömmer du bort att du är i en social situation när något annat fångar ditt intresse?";
	Quiz[86].Text = "Kan du lätt avslöja dolda motiv?";
	Quiz[87].Text = "Har du lätt för att tolka kroppsspråk?";
	Quiz[88].Text = "Har du en monoton röst och/eller svårigheter att finjustera ljudnivå och hastighet när du talar?";
	Quiz[89].Text = "Känner du på dig när det förväntas att du ska be folk om ursäkt?";
	Quiz[90].Text = "Har du en tendens att lätt bli stressad och få panik eller kortslutning i hjärnan i nya och kravfyllda situationer?";
	Quiz[91].Text = "Älskar du att samla på, sortera & organisera saker och/eller göra listor och diagram?";
	Quiz[92].Text = "Föredrar du att använda samma kläder och/eller äta samma mat varje dag?";
	Quiz[93].Text = "Har du starkt behov av att t ex sitta på din favoritplats, åka samma väg eller handla i samma affär varje gång?";
	Quiz[94].Text = "Blir du frustrerad om en för dig viktig aktivitet blir avbruten?";
	Quiz[95].Text = "Blir det kaos inom dig om det händer något oväntat som förändrar din miljö, dina planer eller rutiner?";
	Quiz[96].Text = "Har du vissa enkla, logiska rutiner som gör att du slipper tänka och som det känns bra att följa?";
	Quiz[97].Text = "Har du behov av gosefilt, kramdjur eller liknande?";
	Quiz[98].Text = "Innan du gör något eller går någonstans, behöver du ha en inre bild av vad som kommer att hända så du kan förbereda dig?";
	Quiz[99].Text = "Är du exceptionellt fäst vid vissa saker?";
	Quiz[100].Text= "Har du tvångssyndrom (= tvångstankar eller oemotståndliga, upprepade, irrationella impulser att göra vissa saker)?";
	Quiz[101].Text= "Känns det som du föddes med fel kön?";
	Quiz[102].Text= "Har du ovanliga sexuella preferenser?";
	Quiz[103].Text= "Är du homosexuell eller bisexuell?";
	Quiz[104].Text= "Är du en ledare?";
	Quiz[105].Text= "Tycker du om att tala offentligt?";
	Quiz[106].Text= "Gör du lite allt möligt för uppmärksamhet?";
	Quiz[107].Text= "Är du en bättre lyssnare än talare?";
	Quiz[108].Text= "Blir du uttråkad när det är lugnt och tyst?";
	Quiz[109].Text= "Tänker du innan du agerar?";
	Quiz[110].Text= "Blir du förvirrad av verbala instruktioner - särskilt flera på en gång?";
...
void TQuiz7::SetupCross(TQuiz *QuizI, TQuiz *QuizII, TQuiz *QuizIII, TQuiz *QuizNd, TQuiz *Quiz5, TQuiz *Quiz6)
{
	...
	DefineCross(Quiz6, 69, 5);
	DefineCross(Quiz6, 70, 11);
	DefineCross(QuizNd, 71, 54);
	DefineCross(QuizNd, 72, 56);
	DefineCross(Quiz6, 76, 18);
	DefineCross(Quiz6, 77, 19);
	DefineCross(Quiz6, 78, 21);
	DefineCross(Quiz6, 79, 20);
	DefineCross(Quiz6, 80, 23);
	DefineCross(Quiz6, 81, 26);
	DefineCross(Quiz6, 82, 24);
	DefineCross(Quiz6, 83, 25);
	DefineCross(Quiz6, 84, 27);
	DefineCross(Quiz6, 85, 28);
	DefineCross(Quiz6, 86, 29);
	DefineCross(Quiz6, 87, 30);
	DefineCross(Quiz6, 88, 22);
	DefineCross(Quiz6, 89, 129);
	DefineCross(Quiz6, 90, 55);
	DefineCross(Quiz6, 91, 44);
	DefineCross(Quiz6, 92, 56);
	DefineCross(Quiz6, 93, 57);
	DefineCross(Quiz6, 94, 54);
	DefineCross(Quiz6, 95, 53);
	DefineCross(Quiz6, 96, 58);
	DefineCross(Quiz6, 97, 59);
	DefineCross(Quiz6, 98, 60);
	DefineCross(Quiz6, 99, 62);
	DefineCross(Quiz6, 100, 61);
	DefineCross(Quiz6, 101, 121);
	DefineCross(Quiz6, 102, 122);
	DefineCross(QuizII, 103, 56);
	DefineCross(Quiz6, 104, 112);
	DefineCross(Quiz6, 105, 120);
	DefineCross(Quiz6, 106, 119);
	DefineCross(Quiz6, 107, 109);
	DefineCross(Quiz6, 108, 117);
	DefineCross(Quiz6, 109, 113);
	DefineCross(Quiz6, 110, 125);
	DefineCross(Quiz6, 111, 126);
	DefineCross(Quiz6, 112, 40);
