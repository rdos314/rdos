...
#
##########################################################################*/
void TQuizS4::WriteLongName(TFile &File)
{
	 File.Write("stable version 4");
}
...
void TQuizS4::SetupTexts()
{
  ...
  Quiz[84].MyGroup = GROUP_ENVIRONMENT;
  ...
  Quiz[197].MyGroup = GROUP_NT_HUNTING;
  Quiz[198].MyGroup = GROUP_NT_HUNTING;
  Quiz[199].MyGroup = GROUP_ASPIE_SENSORY;
  Quiz[200].MyGroup = GROUP_MIXED;
  Quiz[201].MyGroup = GROUP_ACTIVITY;
  Quiz[202].MyGroup = GROUP_SOCIAL;
...
void TQuizS4::LoadPopulations()
{
	...
	while (FDataFile.Read(&Row, sizeof(Row)))
	{
		...
		if (Row.Social >= 1)
			SocialPhobia.Add(Row.AsResult, Row.NtResult, DxArr, Row.Gender, Row.Quiz, Row.GroupResult);
