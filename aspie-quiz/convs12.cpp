...
#
# convs12.cpp
# Convert exported quiz-s12 to binary file
#
########################################################################*/
#include <stdio.h>
#include <string.h>
...
const char InsertString[] = "INSERT INTO aspie-quiz-s12 VALUES(";

TFile quizfile("quizs12.bin", 0);
...
char *ProcessRow(char *str)
{
   ...
   int j;
	...
	for (fieldno = 0; fieldno < 205; fieldno++)
	{
		valstr = str;
			...
			*ptr = 0;
			...
			switch (fieldno)
			{
				...
				default:
					...
					if (Row.GiftedResult && i >= 144 && i <= 181)
						Row.Quiz[i]++;
