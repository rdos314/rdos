...
/*##########################################################################
#
#   Name       : TQuizH9::TQuizH9
#
#   Purpose....: Constructor for TQuizH9
#
...
#   Name       : TQuizH9::~TQuizH9
#
#   Purpose....: Destructor for TQuizH9
#
#   In params..: *
#   Out params.: *
#   Returns....: *
...
*   Created....: 96-11-20 le                                                #
*##########################################################################*/
int TQuizH9::GetCatCount(int Question)
{
        return 3;
}
...
#
##########################################################################*/
void TQuizH9::WriteName(TFile &File)
{
         File.Write("H9");
}
...
#
##########################################################################*/
void TQuizH9::WriteLongName(TFile &File)
{
         File.Write("H9");
}
...
#
##########################################################################*/
void TQuizH9::GetReferer(const char *referer, TPopulation *pop)
{
}
...
*   Created....: 96-11-20 le                                                #
*##########################################################################*/
void TQuizH9::ImportMvsp(const char *filename, int PcaType)
{
        char buf[MAX_IN_ROW];
        int size;
        char *rowstr;
...
*   Created....: 96-11-20 le                                                #
*##########################################################################*/
void TQuizH9::WriteEyeResults(const char *filename)
{
    ...
    for (i = 0; i < 28; i++)
    {
        for (j = 0; j < 4; j++)
        {
            val = 10000 * FAspieEyeArr[i][j] / FAspieEyeCount[i];
            sprintf(str, "0.%04d,", val);
            outfile.Write(str);
        }

        for (j = 0; j < 4; j++)
        {
            val = 10000 * FNtEyeArr[i][j] / FNtEyeCount[i];
            sprintf(str, "0.%04d,", val);
            outfile.Write(str);
        }
        str[0] = 0xd;
