...
#define FT_COMPONENT  trace_objs
  ...
  ft_glyphslot_init( FT_GlyphSlot  slot )
  {
    ...
    if ( FT_DRIVER_USES_OUTLINES( driver ) )
      error = FT_GlyphLoader_New( &internal->loader );
...
#define FT_COMPONENT  trace_objs
  ...
  FT_Face_GetVariantSelectors( FT_Face  face )
  {
    ...
    if ( face )
    {
      ...
      if ( charmap != NULL )
      {
        ...
        result = vcmap->clazz->variant_list( vcmap );
      }
    }
                             ...
                             FT_ULong  charcode )
  {
    ...
    if ( face )
    {
      ...
      if ( charmap != NULL )
      {
        ...
        result = vcmap->clazz->charvariant_list( vcmap,
                                                 (FT_UInt32)charcode );
      }
    }
    return result;
                             ...
                             FT_ULong  variantSelector )
  {
    ...
    if ( face )
    {
      ...
      if ( charmap != NULL )
      {
        ...
        result = vcmap->clazz->variantchar_list( vcmap,
                                                 (FT_UInt32)variantSelector );
      }
    }
  ...
  ft_add_renderer( FT_Module  module )
  {
...

    {
      ...
      {
        error = clazz->raster_class->raster_new( &render->raster );
        if ( error )
          goto Fail;
...
#define FREETYPE_VER_FIXED  ( ( (FT_Long)FREETYPE_MAJOR << 16 ) | \
    ...
    if ( FT_MODULE_IS_DRIVER( module ) )
    {
      ...
      if ( FT_DRIVER_USES_OUTLINES( driver ) )
      {
        error = FT_GlyphLoader_New( &driver->glyph_loader );
        if ( error )
          goto Fail;
      }
    }
  ...
  FT_EXPORT_DEF( FT_Error )
  FT_New_Library( FT_Library  *alibrary )
  {
