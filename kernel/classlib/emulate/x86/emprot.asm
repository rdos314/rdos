...
;
	...
	shr bl,2
	and ebx,0Eh
	call dword ptr [2*ebx].EmSaveCrTab
	ret
EmMoveCrReg	Endp
...
;
	...
	shr bl,2
	and ebx,0Eh
	call dword ptr [2*ebx].EmSaveDrTab
	ret
EmMoveDrReg	Endp
...
;
	...
	shr bl,2
	and ebx,0Eh
	call dword ptr [2*ebx].EmLoadDrTab
	and esi,3
	mov esi,dword ptr [4*esi].DwordRegTab
	mov [ebp+esi],eax
	ret
...
;
	mov bl,al
	call LoadWordMemReg
	mov bl,byte ptr [ebp].reg_cr0
	and bl,CR0_PE
	or al,bl
	mov word ptr [ebp].reg_cr0,ax
	ret
EmLmswMem	Endp
...
EmSgdtMem	Proc near
	mov bl,al
	mov eax,[ebp].reg_gdt.d_base
	ror eax,16
	mov dx,ax
	mov ax,word ptr [ebp].reg_gdt.d_limit
	call SaveFwordMem
	ret
EmSgdtMem	Endp
...
EmSidtMem	Proc near
	mov bl,al
	mov eax,[ebp].reg_idt.d_base
	ror eax,16
	mov dx,ax
	mov ax,word ptr [ebp].reg_idt.d_limit
	call SaveFwordMem
	ret
EmSidtMem	Endp
...
;
	and al,NOT 3
	or al,dl
	or byte ptr [ebp].reg_eflags,EFLAGS_ZF		
