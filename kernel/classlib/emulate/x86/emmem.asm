...
MemBxSi	PROC near
	movzx ebx,word ptr [ebp].reg_ebx
	add bx,word ptr [ebp].reg_esi
	movzx esi,byte ptr [ebp].em_sreg
...
MemBpSi	PROC near
	movzx ebx,word ptr [ebp].reg_ebp
	add bx,word ptr [ebp].reg_esi
	movzx esi,byte ptr [ebp].em_sreg
	mov esi,dword ptr [4*esi].SegSsTab
	ret
MemBpSi	ENDP
...
MemSi	PROC near
	movzx ebx,word ptr [ebp].reg_esi
	movzx esi,byte ptr [ebp].em_sreg
	mov esi,dword ptr [4*esi].SegDsTab
	ret
MemSi	ENDP
...
MemSib0	PROC near
	...
	shr di,2
	and edi,0Eh
	mov edi,dword ptr [2*edi].SibIndexTab
	or edi,edi
	jz EmulateError
;
	mov eax,[ebp+edi]
...
LoadByteReg	Proc near
	...
	and bl,0Eh
	movzx esi,bl
	mov esi,dword ptr [2*esi].ByteRegTab
	mov al,[ebp+esi]
	ret
LoadByteReg	Endp
...
LoadWordReg	Proc near
	...
	and bl,0Eh
	movzx esi,bl
	mov esi,dword ptr [2*esi].WordRegTab
	mov ax,[ebp+esi]
	ret
LoadWordReg	Endp
...
LoadDwordReg	Proc near
	...
	and bl,0Eh
	movzx esi,bl
	mov esi,dword ptr [2*esi].DwordRegTab
	mov eax,[ebp+esi]
	ret
LoadDwordReg	Endp
