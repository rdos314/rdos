...
device_name         DB 16 DUP(?)
device_read         DD ?,?
device_write        DD ?,?

device_struc    ENDS

std_device_process_seg      STRUC

con_buf             DB 256 DUP(?)
con_buf_start       DW ?
con_buf_end         DW ?

std_device_process_seg      ENDS

data    SEGMENT byte public 'DATA'

device_last         DW ?

devices             DB 32*16 DUP(?)
...
;       
;
;           NAME:           InstallDeviceFile
;
;           DESCRIPTION:    Install device file
;
;           PARAMETERS:     DS:ESI       Device read
;                           ES:EDI       Device write
;                           FS:EBX       Device name
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

install_device_file_name    DB 'Install Device File',0

install_device_file PROC far
    push ds
    push es
    push fs
    pushad
...
;
    mov ax,SEG data
    mov fs,ax
    mov bx,fs:device_last
    mov fs:[bx].device_read,esi
    mov fs:[bx+4].device_read,ds
    mov fs:[bx].device_write,edi
    mov fs:[bx+4].device_write,es
;
    mov ax,SEG data
    mov es,ax
    mov di,bx
...
;
    ...
    HookCreateProcess
;
    mov esi,OFFSET install_device_file
    mov edi,OFFSET install_device_file_name
    mov ax,install_device_file_nr
    RegisterOsGate
;
...
;
    mov ax,SEG data
    mov ds,ax    
    mov ds:device_last,OFFSET devices
;
    mov ax,cs
    mov ds,ax
    mov es,ax
    mov fs,ax
;    
    mov ebx,OFFSET con_device
    mov esi,OFFSET con_read
