...
;
;	Parameters:		ES:DI	Cache entry callback
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
init_cache	PROC near
	...
	mov es,ax
;
	mov si,OFFSET hook_ip_cache
	mov di,OFFSET hook_ip_cache_name
	xor cl,cl
	mov ax,hook_ip_cache_nr
	RegisterOsGate
;
	mov si,OFFSET allocate_ip_cache_mem
	mov di,OFFSET allocate_ip_cache_mem_name
	xor cl,cl
	mov ax,allocate_ip_cache_mem_nr
	RegisterOsGate
;
	mov si,OFFSET lookup_ip_cache
	mov di,OFFSET lookup_ip_cache_name
	xor cl,cl
	mov ax,lookup_ip_cache_nr
	RegisterOsGate
;
	mov si,OFFSET get_host_timeout
	mov di,OFFSET get_host_timeout_name
	xor cl,cl
	mov ax,get_host_timeout_nr
	RegisterOsGate
;
	mov si,OFFSET update_round_trip_time
	mov di,OFFSET update_round_trip_time_name
	xor cl,cl
