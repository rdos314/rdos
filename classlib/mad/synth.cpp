...
void TMadSynth::Mute()
{
  ...
  for (ch = 0; ch < 2; ++ch)
  {
	for (s = 0; s < 16; ++s)
	{
	  for (v = 0; v < 8; ++v)
	  {
		filter[ch][0][0][s][v] = filter[ch][0][1][s][v] = 0;
		filter[ch][1][0][s][v] = filter[ch][1][1][s][v] = 0;
	  }
	}
  }
}
...
void TMadSynth::FullSynth(TMadFrame const *frame, unsigned int nch, unsigned int ns)
{
  ...
  for (ch = 0; ch < nch; ++ch)
  {
	...
	for (s = 0; s < ns; ++s)
	{
	  dct32((*sbsample)[s], lphase >> 1,
		(*lfilter)[0][lphase & 1], (*lfilter)[1][lphase & 1]);
	  ...
	  fe = &(*lfilter)[0][ lphase & 1][0];
	  fx = &(*lfilter)[0][~lphase & 1][0];
	  fo = &(*lfilter)[1][~lphase & 1][0];
 ...
 */
void TMadSynth::Synth(TMadFrame const *frame)
{
  unsigned int nch, ns;
  ...
  if (frame->options & MAD_OPTION_HALFSAMPLERATE)
  {
	...
	HalfSynth(frame, nch, ns);
  }
  else
	FullSynth(frame, nch, ns);
