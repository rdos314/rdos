...
void LonEventHandler(void)
{
    ...
    if (LdvGetMsg(&pSmipMsg) == LonApiNoError)
    {
        ...
        unsigned char Priority = (EXPMSG.LON_EXPMSG_PRIORITY_FIELD & LON_EXPMSG_PRIORITY_MASK) >> LON_EXPMSG_PRIORITY_SHIFT;
        unsigned char Tag = (EXPMSG.LON_EXPMSG_TAG_FIELD & LON_EXPMSG_TAG_MASK) >> LON_EXPMSG_TAG_SHIFT;
        unsigned char Service = (EXPMSG.LON_EXPMSG_SERVICE_FIELD & LON_EXPMSG_SERVICE_MASK) >> LON_EXPMSG_SERVICE_SHIFT;
        unsigned char MsgType = (EXPMSG.LON_EXPMSG_MSGTYPE_FIELD & LON_EXPMSG_MSGTYPE_MASK) >> LON_EXPMSG_MSGTYPE_SHIFT;
        unsigned char Auth = (EXPMSG.LON_EXPMSG_AUTHENTICATED_FIELD & LON_EXPMSG_AUTHENTICATED_MASK) >> LON_EXPMSG_AUTHENTICATED_SHIFT;
        unsigned char CompletionCode = (EXPMSG.LON_EXPMSG_COMPLETIONCODE_FIELD & LON_EXPMSG_COMPLETIONCODE_MASK) >> LON_EXPMSG_COMPLETIONCODE_SHIFT;

        correlator.LON_CORRELATOR_PRIORITY_FIELD = ((correlator.LON_CORRELATOR_PRIORITY_FIELD) & ~LON_CORRELATOR_PRIORITY_MASK) | (Priority << LON_CORRELATOR_PRIORITY_SHIFT);
        correlator.LON_CORRELATOR_TAG_FIELD = ((correlator.LON_CORRELATOR_TAG_FIELD) & ~LON_CORRELATOR_TAG_MASK) | (Tag << LON_CORRELATOR_TAG_SHIFT);
        correlator.LON_CORRELATOR_SERVICE_FIELD = ((correlator.LON_CORRELATOR_SERVICE_FIELD) & ~LON_CORRELATOR_SERVICE_MASK) | (Service << LON_CORRELATOR_SERVICE_SHIFT);
        ...
        switch (pSmipMsg->Header.Command)
        {
            case ((unsigned char) LonNiComm | (unsigned char) LonNiIncoming):
            {
                ...
                if (MsgType == LonMessageNv)
                {
                    /* Process NV messages */
                    char NvPoll = (NVMSG.LON_NVMSG_NVPOLL_FIELD & LON_NVMSG_NVPOLL_MASK) >> LON_NVMSG_NVPOLL_SHIFT;
                    if (NvPoll)
                    {
            ...
            case ((unsigned char) LonNiComm | (unsigned char) LonNiResponse):
            {
                if (CompletionCode)
                {
                    ...
                    if (MsgType == LonMessageNv)
                    {
                        unsigned char NvCompletionCode = (NVMSG.LON_NVMSG_COMPLETIONCODE_FIELD & LON_NVMSG_COMPLETIONCODE_MASK) >> LON_NVMSG_COMPLETIONCODE_SHIFT;
                        LonNvUpdateCompleted(NVMSG.Index, (bool)(NvCompletionCode == LonCompletionSuccess));
                    }
                    else
                    {
                            LonMsgCompleted(Tag, (bool)CompletionCode);
                    }
                }
                else
                {
                            ...
                            if (CurrentNmNdStatus == NM_PENDING)
                            {
                                ...
                                switch ((EXPMSG.Code & LON_NM_OPCODE_MASK) | LON_NM_OPCODE_BASE)
                                {
                                    case LonNmQueryDomain:
                                        ...
                                        LonDomainConfigReceived((LonDomain*) &(EXPMSG.Data), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNmQueryDomain)));
                                        break;
                                    case LonNmQueryNvConfig:
                                        /* Query Nv Config response    */
                                        if (EXPMSG.Length == sizeof(EXPMSG.Code) + sizeof(LonNvConfig)) 
                                            LonNvConfigReceived((LonNvConfig*) &(EXPMSG.Data), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNmQueryNvConfig)));
                                        else 
                                            LonAliasConfigReceived((LonAliasConfig*) &(EXPMSG.Data), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNmQueryNvConfig)));
                                        break;
                                    case LonNmQueryAddr:
                                        /* Query Address response    */
                                        LonAddressConfigReceived((LonAddress*) &(EXPMSG.Data), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNmQueryAddr)));
                                        break;
                                    case LonNmReadMemory:
                                        /* Read of configuration data response    */
                                        LonConfigDataReceived((const LonConfigData* const)&(EXPMSG.Data.Data), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNmReadMemory)));
                                        break;
                                    default:
                                        break;
                                }
                            ...
                            else if (CurrentNmNdStatus == ND_PENDING)
                            {
                                ...
                                switch ((EXPMSG.Code & LON_ND_OPCODE_MASK) | LON_ND_OPCODE_BASE)
                                {
                                    case LonNdQueryStatus:
                                        ...
                                        LonStatusReceived(&(EXPMSG.Data.QueryStatusResponse.Status), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNdQueryStatus)));
                                        break;
                                    case LonNdQueryXcvr:
                                        /* Query Transceiver Status response */
                                        LonTransceiverStatusReceived(&(EXPMSG.Data.QueryXcvrStatusResponse.Status), (bool)(EXPMSG.Code == LON_NM_SUCCESS(LonNdQueryXcvr)));
                                        break;
                                    default:
                                        break;
                                }
            ...
            case LonNiUsop:
                ...
                switch (pSmipMsg->Payload[0])
                {
                    ...
                    case LonUsopNvIsBound:
                        LonNvIsBoundReceived(pSmipMsg->Payload[1], (bool) pSmipMsg->Payload[2]);
                        break;
                        
                    case LonUsopMtIsBound:
                        LonMtIsBoundReceived(pSmipMsg->Payload[1], (bool) pSmipMsg->Payload[2]);
                        break;
 ...
 */
const LonApiError LonSendMsg(const unsigned tag, const bool priority, const LonServiceType serviceType, 
            const bool authenticated, const LonSendAddress* const pDestAddr,
            const unsigned char code, const unsigned char* const pData, const unsigned length)
{
    LonSmipMsg* pSmipMsg = NULL;
    LonSicb * pScib = (LonSicb*)pSmipMsg->Payload;
 ...
 * This function is part of the optional utility API (LON_UTILITY_FUNCTIONS).
 */
const LonApiError LonQueryAppSignature(bool bInvalidate)
{
    unsigned char data[2];
